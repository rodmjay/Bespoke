    ...
    public override void Configure(EntityTypeBuilder<CustomerAccount> builder)
    {
        ...
        builder.Property(x => x.IsInternal)
            .HasComputedColumnSql(
                @"case when [AccountManagerOrganizationId]=[CustomerOrganizationId] then cast(1 as bit) else cast(0 as bit) end");

        builder.Property(x => x.IsCorporate)
            .HasComputedColumnSql(
                @"case when [AccountManagerOrganizationId]=[CustomerOrganizationId] AND [AccountManagerId]=[CustomerId] then cast(1 as bit) else cast(0 as bit) end");
