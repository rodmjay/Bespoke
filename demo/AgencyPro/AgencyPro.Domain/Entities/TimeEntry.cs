    ...
    public override void Configure(EntityTypeBuilder<TimeEntry> builder)
    {
        ...
        builder.Property(x => x.TotalCustomerAmount)
            .HasComputedColumnSql(
                $@"([InstantSystemStream]+[InstantAccountManagerStream]+[InstantProjectManagerStream]+[InstantMarketerStream]+[InstantRecruiterStream]+[InstantContractorStream]+[InstantAgencyStream]+[InstantRecruitingAgencyStream]+[InstantMarketingAgencyStream])*{totalHoursComputation}");

        builder.Property(x => x.TotalRecruitingStream)
            .HasComputedColumnSql(
                $@"([InstantRecruitingAgencyStream]*{totalHoursComputation})+([InstantRecruiterStream]*{totalHoursComputation})");
        builder.Property(x => x.TotalMarketingStream)
            .HasComputedColumnSql(
                $@"([InstantMarketingAgencyStream]*{totalHoursComputation})+([InstantMarketerStream]*{totalHoursComputation})");
