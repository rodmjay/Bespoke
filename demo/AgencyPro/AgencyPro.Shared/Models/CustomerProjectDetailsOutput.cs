...
public class CustomerProjectDetailsOutput : CustomerProjectOutput
{
    ...
    public IDictionary<StoryStatus, int> StoryStatus =>
        Stories.GroupBy(a => a.Status).ToDictionary(b => b.Key, b => b.Count());

    public IDictionary<ContractStatus, int> ContractStatus =>
        Contracts.GroupBy(a => a.Status).ToDictionary(b => b.Key, b => b.Count());
