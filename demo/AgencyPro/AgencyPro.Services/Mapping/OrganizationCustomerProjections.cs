...
public class OrganizationCustomerProjections : Profile
{
    public OrganizationCustomerProjections()
    {
        ...
        CreateMap<OrganizationCustomer, OrganizationCustomerOutput>()
            ...
            .ForMember(x => x.DisplayName, opt => opt.MapFrom(x => x.Customer.Person.FirstName))
        ...
        CreateMap<OrganizationCustomer, CustomerCounts>()
            .ForMember(x => x.Proposals,
                opt => opt.MapFrom(x => x.Projects.Count(y =>
                    y.Proposal != null && y.Proposal.Status != ProposalStatus.Draft &&
                    y.Proposal.Status != ProposalStatus.Rejected)))
            .ForMember(x => x.Accounts, opt => opt.MapFrom(x => x.Accounts.Count))
            .ForMember(x => x.FinancialAccounts,
                opt => opt.MapFrom(x => x.Organization.OrganizationFinancialAccount == null ? 0 : 1))
            .ForMember(x => x.Projects,
                opt => opt.MapFrom(x =>
                    x.Projects.Count(p => p.Proposal != null && p.Proposal.Status == ProposalStatus.Accepted)))
            .ForMember(x => x.Contracts,
                opt => opt.MapFrom(x => x.Projects.SelectMany(y => y.Contracts.Where(z =>
                    z.Project.Proposal != null && z.Project.Proposal.Status == ProposalStatus.Accepted)).Count()))
            .ForMember(x => x.Invoices,
                opt => opt.MapFrom(x =>
                    x.Invoices.Count(y => y.Invoice.Status == "open" || y.Invoice.Status == "paid")))
            .ForMember(x => x.WorkOrders,
                opt => opt.MapFrom(x => x.Organization.BuyerWorkOrders.Count(y => y.Status != OrderStatus.Rejected)));
    }
}