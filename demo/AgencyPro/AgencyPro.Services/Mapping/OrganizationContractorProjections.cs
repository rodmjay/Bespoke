...
public partial class OrganizationContractorProjections : Profile
{
    public OrganizationContractorProjections()
    {
        ...
        CreateMap<OrganizationContractor, OrganizationContractorOutput>()
            ...
            .ForMember(x => x.RecruiterName, opt => opt.MapFrom(x => x.Contractor.Recruiter.Person.FirstName))
            ...
            .ForMember(x => x.RecruiterOrganizationName,
                opt => opt.MapFrom(x => x.Contractor.OrganizationRecruiter.Organization.Name))
            .ForMember(x => x.RecruiterOrganizationImageUrl,
                opt => opt.MapFrom(x => x.Contractor.OrganizationRecruiter.Organization.ImageUrl))
        ...
        CreateMap<OrganizationContractor, OrganizationContractorStatistics>()
            ...
            .ForMember(x => x.TimeEntryHours,
                opt => opt.MapFrom(y =>
                    y.TimeEntries.GroupBy(z => z.Status).ToDictionary(a => a.Key, a => a.Sum(b => b.TotalHours))))
            .ForMember(x => x.TimeEntryEarnings,
                opt => opt.MapFrom(y =>
                    y.TimeEntries.GroupBy(z => z.Status)
                        .ToDictionary(a => a.Key, a => a.Sum(b => b.TotalContractorStream))))
        ...
        CreateMap<OrganizationContractor, ContractorCounts>()
            .ForMember(x => x.TimeEntries,
                opt => opt.MapFrom(x => x.TimeEntries.Count(a => a.Status == TimeStatus.Logged)))
