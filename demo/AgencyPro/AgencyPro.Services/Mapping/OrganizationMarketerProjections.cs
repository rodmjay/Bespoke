...
public class OrganizationMarketerProjections : Profile
{
    public OrganizationMarketerProjections()
    {
        CreateMap<OrganizationMarketer, OrganizationMarketerOutput>()
            .ForMember(x => x.DisplayName, opt => opt.MapFrom(x => x.Marketer.Person.FirstName))
        ...
        CreateMap<OrganizationMarketer, MarketerCounts>()
            ...
            .ForMember(x => x.ChannelPartners,
                opt => opt.MapFrom(x =>
                    x.Organization.MarketingOrganization.MarketingAgreements.Count(y =>
                        y.Status == AgreementStatus.Approved)))
