...
public partial class OrganizationProjections : Profile
{
    public OrganizationProjections()
    {
        ...
        CreateMap<Organization, AgencyOwnerOrganizationDetailsOutput>()
            ...
            .ForMember(x => x.FinancialAccount,
                opt => opt.MapFrom(x => x.OrganizationFinancialAccount.FinancialAccount))
        ...
        CreateMap<Organization, AgencyOwnerProviderOrganizationDetailsOutput>()
            .ForMember(x => x.AvailablePaymentTerms,
                opt => opt.MapFrom(x => x.Category.AvailablePaymentTerms.Select(y => y.PaymentTerm)))
            .ForMember(x => x.AvailableBillingCategories,
                opt => opt.MapFrom(x => x.Category.AvailableBillingCategories.Select(y => y.BillingCategory)))
            ...
            .ForMember(x => x.Skills,
                opt => opt.MapFrom(x => x.ProviderOrganization.Skills.ToDictionary(z => z.SkillId, y => y.Priority)))
            .ForMember(x => x.PaymentTerms,
                opt => opt.MapFrom(x => x.PaymentTerms.ToDictionary(z => z.PaymentTermId, y => y.IsDefault)))
            .ForMember(x => x.BillingCategories,
                opt => opt.MapFrom(x => x.BillingCategories.Select(y => y.BillingCategory)));

        CreateMap<Organization, AgencyOwnerCounts>()
            .ForMember(x => x.TimeEntries,
                opt => opt.MapFrom(x => x.ProviderOrganization.Projects.SelectMany(y =>
                    y.Contracts.SelectMany(z => z.TimeEntries).Where(a => a.Status == TimeStatus.Logged)).Count()))
            .ForMember(x => x.Stories,
                opt => opt.MapFrom(x => x.ProviderOrganization.Projects.SelectMany(y => y.Stories).Count()))
            .ForMember(x => x.Proposals,
                opt => opt.MapFrom(x =>
                    x.ProviderOrganization.Projects.Count(y =>
                        y.Proposal != null && y.Proposal.Status != ProposalStatus.Rejected)))
            ...
            .ForMember(x => x.RecruitingContracts,
                opt => opt.MapFrom(x => x.RecruitingOrganization.RecruiterContracts.Count))
            .ForMember(x => x.MarketingContracts,
                opt => opt.MapFrom(x => x.MarketingOrganization.MarketerContracts.Count))
            .ForMember(x => x.ProviderMarketingPartner,
                opt => opt.MapFrom(x =>
                    x.ProviderOrganization.MarketingAgreements.Count(y => y.Status != AgreementStatus.Rejected)))
            .ForMember(x => x.ProviderRecruitingPartner,
                opt => opt.MapFrom(x =>
                    x.ProviderOrganization.RecruitingAgreements.Count(y => y.Status != AgreementStatus.Rejected)))
            .ForMember(x => x.MarketingPartner,
                opt => opt.MapFrom(x =>
                    x.MarketingOrganization.MarketingAgreements.Count(y => y.Status != AgreementStatus.Rejected)))
            .ForMember(x => x.RecruitingPartner,
                opt => opt.MapFrom(x =>
                    x.RecruitingOrganization.RecruitingAgreements.Count(y => y.Status != AgreementStatus.Rejected)))
            .ForMember(x => x.Leads, opt => opt.MapFrom(x =>
                x.ProviderOrganization.Leads.Count(y => y.Status == LeadStatus.New)
                + x.Leads.Count(y => y.IsInternal == false)))
            .ForMember(x => x.Candidates,
                opt => opt.MapFrom(x => x.ProviderOrganization.Candidates.Count(y => y.Status == CandidateStatus.New)))
            .ForMember(x => x.WorkOrders,
                opt => opt.MapFrom(x => x.ProviderOrganization.WorkOrders.Count(y => y.Status != OrderStatus.Draft)))
            .ForMember(x => x.Invoices,
                opt => opt.MapFrom(x => x.ProviderOrganization.Projects.SelectMany(y => y.Invoices).Count()));
