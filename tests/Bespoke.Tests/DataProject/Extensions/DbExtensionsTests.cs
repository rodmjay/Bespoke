...
namespace Bespoke.Tests.DataProject.Extensions
{
    ...
    public class DbExtensionsTests
    {
        ...
        public void BuildLikeExpression_SingleKeyword_BuildsCorrectExpression()
        {
            ...
            Assert.That(expression.ToString(), Is.EqualTo("x => value(Microsoft.EntityFrameworkCore.DbFunctions).Like(x.Name, \"%test%\")"));
        }
        ...
        public void BuildLikeExpression_MultipleKeywords_BuildsCorrectExpression()
        {
            ...
            // Assert
            Assert.That(expression, Is.Not.Null);
            Assert.That(expression.ToString(), Is.EqualTo("x => (value(Microsoft.EntityFrameworkCore.DbFunctions).Like(x.Name, \"%test%\")) OrElse value(Microsoft.EntityFrameworkCore.DbFunctions).Like(x.Name, \"%example%\")"));
        }
